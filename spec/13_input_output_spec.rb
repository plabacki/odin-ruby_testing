...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_continue) {described_class.new(4,'6')}
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      ...
      invalid_input_two = 'b'
      valid_input = '3'
      allow(game_loop).to receive(:player_input).and_return(invalid_input_one,invalid_input_two,valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:hard_game){described_class.new(5,'5',5)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        hard_phrase = "That was hard. It took you 5 guesses!\n"
        expect {hard_game.final_message}.to output(hard_phrase).to_stdout
      end
    end
  end
end
