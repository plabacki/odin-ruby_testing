...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_drink) {described_class.new('milk')}
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_drink.type).to eq('milk')
      end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:another_drink) {described_class.new('water')}
      # remove the 'x' before running this test
      it 'is full' do
        expect(another_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:another_drink) {described_class.new('water',8)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(another_drink).not_to be_full
      end
    end
  end
end
