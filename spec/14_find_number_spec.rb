...
describe FindNumber do
  ...
  describe '#game_over?' do
    let(:correct_number){double('correct_number',value:3)}
    context 'when guess and random_number are equal' do
      ...
      subject(:game_guessing){described_class.new(2,4,correct_number,3)}
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_guessing){described_class.new(2,4,correct_number,5)}
      it 'is not game over' do
        expect(game_guessing).to_not be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:another_guess_game) {described_class.new(5,8,number_range,7)}
      before do
        another_guess_game.update_range
      end  
      it 'updates min to the same value as max' do
        expect(another_guess_game.min).to eq(8)
      end

      it 'does not update max' do
        expect(another_guess_game.max).to eq(8)
      end
    end
  end
end
