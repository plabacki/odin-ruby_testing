...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 13 and random number is 6'do
      subject(:new_game) {described_class.new(6)}  
      it 'return 7'do
        previous_step = 13
        result = new_game.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 10' do
      subject(:new_game) {described_class.new(10)}
      it 'will return 7' do
    ...
    context 'when the random number is 99' do
      subject(:new_game) {described_class.new(99)}
    ...
    context 'when the random number is 5' do
      subject(:new_game) {described_class.new(5)}
      # remove the 'x' before running this test
